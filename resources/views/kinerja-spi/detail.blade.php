    ...
    <main class="container">
        <div class="main-content">
            <div class="left-column">
                <!-- Ringkasan Kinerja SPI -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-info-circle"></i> Ringkasan Kinerja SPI</h2>
                        <a href="#" class="btn-primary openPopup" data-id="{{ $kinerja->id }}">
                            Lihat Selengkapnya
                        </a>
                    </div>
                    <p class="abstract-content">
                        {{ $kinerja->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                    </p>
                </div>

                <!-- Metadata Kinerja SPI -->
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-tags"></i> Metadata Kinerja SPI</h2>
                    </div>
                    <dl class="metadata-grid">
                        <div class="metadata-item">
                            <dt>Tipe Dokumen</dt>
                            <dd>{{ $kinerja->tipe_dokumen ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Judul</dt>
                            <dd>{{ $kinerja->judul ?? '-' }}</dd>
                        ...
                        <div class="metadata-item">
                            <dt>Bentuk</dt>
                            <dd>{{ $kinerja->bentuk ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk Singkat</dt>
                            <dd>{{ $kinerja->bentuk_singkat ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tahun</dt>
                            <dd>{{ $kinerja->tahun_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tempat Penetapan</dt>
                            <dd>{{ $kinerja->tempat_penetapan ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Penetapan</dt>
                            <dd>{{ $kinerja->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Pengundangan</dt>
                            <dd>{{ $kinerja->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Berlaku</dt>
                            <dd>{{ $kinerja->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Sumber</dt>
                            <dd>{{ $kinerja->sumber ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Subjek</dt>
                            <dd>{{ $kinerja->subjek ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Status</dt>
                            <dd>{{ $kinerja->status ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bahasa</dt>
                            <dd>{{ $kinerja->bahasa ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Lokasi</dt>
                            <dd>{{ $kinerja->lokasi ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bidang</dt>
                            <dd>{{ $kinerja->bidang ?? '-' }}</dd>
                        </div>
                    </dl>
                    <div class="social-share">
                        <span class="share-text">Halaman ini telah diakses {{ $kinerja->views }} kali</span>
                ...
                <div class="card" style="margin-top: 2rem;">
                    ...
                    <p class="abstract-content">{{ $kinerja->uji_materi ?? 'Belum Tersedia' }}</p>
                </div>
            </div>
            ...
            <div class="right-column">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-file-download"></i> File Kinerja SPI</h2>
                    </div>
                    <div class="file-card-content">
                        <i class="fas fa-file-pdf file-icon"></i>
                        <div class="file-name">{{ $kinerja->file_pdf ?? 'Tidak ada file' }}</div>
                        <div class="btn-group">
                            @php
                                $filePath = $kinerja->file_pdf
                                    ? asset('storage/' . trim($kinerja->file_pdf))
                                    : null;
                            @endphp
                    ...
                    <div class="card" style="margin-top: 2rem;">
                        <div class="card-header">
                            <h2><i class="fas fa-check-circle"></i> Status Kinerja SPI</h2>
                        </div>
                            ...
                            @if($kinerja->mencabut)
                                <div class="status-item">
                                    ...
                                    <div>
                                        ...
                                        <ul>
                                            @foreach(explode("\n", str_replace("\r", '', $kinerja->mencabut)) as $item)
                                                <li>{{ $item }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            openPopupBtn.forEach(link => {
                link.addEventListener('click', function (e) {
                    e.preventDefault();
                    const kinerjaId = this.dataset.id;

                    fetch(`/kinerja-spi/${kinerjaId}/detail`)
                        .then(res => res.json())
                        .then(data => {
                        ...
                        .catch(err => console.error('Error fetching kinerja:', err));
                });
            });
