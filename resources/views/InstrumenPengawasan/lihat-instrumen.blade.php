...
<body>
    ...
    <div class="search-wrapper">
        <form action="{{ route('instrumen.search') }}" method="GET" class="search-form" style="display: contents;">
            <div class="input-group">
                <input type="text" name="keyword" placeholder="Cari Instrumen ..." value="{{ request('keyword') }}">
            </div>
    ...
    <div id="advModal" class="modal">
        <div class="modal-box">
            ...
            <form class="adv-form" action="{{ route('instrumen.search') }}" method="GET">
        ...
        <div class="document-grid">
            @forelse($instrumens as $Instrumen)
                <div class="document-card">
                    ...
                    <div class="card-body">
                        <h3 class="card-title">{{ $Instrumen->judul }}</h3>
                        <p class="card-subtitle">Tahun: {{ $Instrumen->tahun ?? '-' }}</p>
                    </div>
                    <div class="card-actions">
                        <a href="{{ route('instrumen.show', $Instrumen->id) }}" class="action-btn view-btn">
                            <i class="fa fa-eye"></i> Lihat
                        </a>

                        @if(Auth::check() && Auth::user()->role === 'admin')
                            <a href="{{ route('instrumen.edit', $Instrumen->id) }}" class="action-btn edit-btn">
                                <i class="fa fa-edit"></i> Edit
                            </a>

                            <form action="{{ route('instrumen.destroy', $Instrumen->id) }}" method="POST" class="inline-form"
            ...
            @empty
                <p class="no-data-message">Tidak ada data Instrumen Pengawasan {{ ucfirst($jenis) }}.</p>
            @endforelse
        </div>
    </div>
