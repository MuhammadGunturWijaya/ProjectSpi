...
@section('content')
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-primary"><i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $PosAp->judul }}</h2>
            <p class="text-muted fs-6">Perbarui informasi dan metadata dokumen POS AP dengan mudah.</p>
        </div>
        ...
        <form action="{{ route('posAp.update', $PosAp->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="judul" name="judul"
                                    value="{{ old('judul', $PosAp->judul) }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="tahun" class="form-label fw-semibold">Tahun <span
                                        class="text-danger">*</span></label>
                                <input type="number" class="form-control" id="tahun" name="tahun"
                                    value="{{ old('tahun', $PosAp->tahun) }}" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                @if($PosAp->file_pdf)
                                    <p class="mt-2 text-muted"><i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                        <a href="{{ asset('storage/' . $PosAp->file_pdf) }}"
                                            target="_blank">{{ $PosAp->file_pdf }}</a>
                                    </p>
                                @endif
                            </div>
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="mencabut" name="mencabut"
                                    rows="3">{{ old('mencabut', $PosAp->mencabut) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
