    ...
    <div class="legal-search-container">
        <div class="search-header-panel">
            <div class="search-info">
                <h1>PENCARIAN <span class="highlight-text">POS AP</span></h1>
                <p class="search-summary">
                    Menemukan
                    <span class="result-count">{{ $posaps->total() }}</span>
                    Pos AP
                    @if($keyword)
        ...
        <div class="results-grid">
            @forelse($posaps as $item)
                <div class="result-card">
                    ...
                    <h3 class="card-title">
                        <a href="{{ route('posap.show', $item->id) }}" class="detail-link">{{ $item->judul }}</a>
                    </h3>
            ...
            @empty
                <p>Tidak ada Pos AP yang sesuai dengan pencarian.</p>
            @endforelse
        </div>
        ...
        @if ($posaps->hasPages())
            <div class="pagination-bar">
                {{-- Tombol First --}}
                @if (!$posaps->onFirstPage())
                    <a href="{{ $posaps->url(1) }}" class="page-link first">First</a>
                @else
                ...
                <div class="page-numbers">
                    @foreach ($posaps->getUrlRange(1, $posaps->lastPage()) as $page => $url)
                        @if ($page == $posaps->currentPage())
                ...
                {{-- Tombol Next --}}
                @if ($posaps->hasMorePages())
                    <a href="{{ $posaps->nextPageUrl() }}" class="page-link next">Next</a>
                @else
