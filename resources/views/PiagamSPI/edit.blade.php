    ...
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-primary">
                <i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $piagam->judul }}
            </h2>
            <p class="text-muted fs-6">Perbarui informasi dan metadata dokumen piagam SPI dengan mudah.</p>
        </div>
        ...
        <form action="{{ route('piagam.update', $piagam->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="judul" name="judul"
                                    value="{{ old('judul', $piagam->judul) }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="tahun" class="form-label fw-semibold">Tahun <span
                                        class="text-danger">*</span></label>
                                <input type="number" class="form-control" id="tahun" name="tahun"
                                    value="{{ old('tahun', $piagam->tahun) }}" required>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="row g-3">
                                ...
                                @foreach($metaFields as $field)
                                    <div class="col-md-6">
                                        ...
                                        <input type="{{ $field['type'] ?? 'text' }}" class="form-control"
                                            ...
                                            value="{{ old($field['name'], $piagam->{$field['name']} && !is_string($piagam->{$field['name']}) ? $piagam->{$field['name']}->format('Y-m-d') : $piagam->{$field['name']}) }}">
                                    </div>
                                @endforeach
                                ...
                                <div class="col-12">
                                    ...
                                    @if($piagam->file_pdf)
                                        <p class="mt-2 text-muted"><i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                            <a href="{{ asset('storage/' . $piagam->file_pdf) }}"
                                                target="_blank">{{ $piagam->file_pdf }}</a>
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control" id="mencabut" name="mencabut"
                                        rows="3">{{ old('mencabut', $piagam->mencabut) }}</textarea>
                                </div>
                            </div>
