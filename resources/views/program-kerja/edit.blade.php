    ...
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-primary">
                <i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $programKerja->judul }}
            </h2>
            <p class="text-muted fs-6">Perbarui informasi dan metadata dokumen Program Kerja SPI dengan mudah.</p>
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="kata_kunci" name="kata_kunci"
                                    value="{{ old('kata_kunci', $programKerja->kata_kunci) }}">
                            </div>
                            <div class="mb-3">
                                <label for="abstrak" class="form-label fw-semibold">Abstrak</label>
                                <textarea class="form-control" id="abstrak" name="abstrak"
                                    rows="4">{{ old('abstrak', $programKerja->abstrak) }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label for="catatan" class="form-label fw-semibold">Catatan</label>
                                <textarea class="form-control" id="catatan" name="catatan"
                                    rows="3">{{ old('catatan', $programKerja->catatan) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                @foreach($metaFields as $field)
                                    <div class="col-md-6">
                                        ...
                                        <input type="{{ $field['type'] ?? 'text' }}" class="form-control"
                                            ...
                                            value="{{ old($field['name'], $programKerja->{$field['name']} ? date('Y-m-d', strtotime($programKerja->{$field['name']})) : '') }}">
                                ...
                                <div class="col-12">
                                    ...
                                    @if($programKerja->file_pdf)
                                        <p class="mt-2 text-muted"><i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                            <a href="{{ asset('storage/' . $programKerja->file_pdf) }}"
                                                target="_blank">{{ $programKerja->file_pdf }}</a>
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control" id="mencabut" name="mencabut"
                                        rows="3">{{ old('mencabut', $programKerja->mencabut) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end mt-5">
                                ...
                                <a href="{{ route('programKerja.index') }}"
