            ...
            <div class="left-column">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-info-circle"></i> Ringkasan Peningkatan Kualitas</h2>
                        <a href="#" class="btn-primary openPopup" data-id="{{ $peningkatan->id }}">
                            Lihat Selengkapnya
                        </a>
                    </div>
                    <p class="abstract-content">
                        {{ $peningkatan->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                    </p>
                </div>

                <!-- Metadata Peningkatan Kualitas -->
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-tags"></i> Metadata Peningkatan Kualitas</h2>
                    </div>
                    <dl class="metadata-grid">
                        <div class="metadata-item">
                            <dt>Tipe Dokumen</dt>
                            <dd>{{ $peningkatan->tipe_dokumen ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Judul</dt>
                            <dd>{{ $peningkatan->judul_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>T.E.U.</dt>
                            <dd>{{ $peningkatan->teu ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Nomor</dt>
                            <dd>{{ $peningkatan->nomor ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk</dt>
                            <dd>{{ $peningkatan->bentuk ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk Singkat</dt>
                            <dd>{{ $peningkatan->bentuk_singkat ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tahun</dt>
                            <dd>{{ $peningkatan->tahun_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tempat Penetapan</dt>
                            <dd>{{ $peningkatan->tempat_penetapan ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Penetapan</dt>
                            <dd>{{ $peningkatan->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Pengundangan</dt>
                            <dd>{{ $peningkatan->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Berlaku</dt>
                            <dd>{{ $peningkatan->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Sumber</dt>
                            <dd>{{ $peningkatan->sumber ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Subjek</dt>
                            <dd>{{ $peningkatan->subjek ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Status</dt>
                            <dd>{{ $peningkatan->status ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bahasa</dt>
                            <dd>{{ $peningkatan->bahasa ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Lokasi</dt>
                            <dd>{{ $peningkatan->lokasi ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bidang</dt>
                            <dd>{{ $peningkatan->bidang ?? '-' }}</dd>
                        </div>
                    </dl>
                    <div class="social-share">
                        <span class="share-text">
                            Halaman ini telah diakses {{ $peningkatan->view_count ?? 0 }} kali
                        </span>
                ...
                <div class="card" style="margin-top: 2rem;">
                    ...
                    <p class="abstract-content">{{ $peningkatan->uji_materi ?? 'Belum Tersedia' }}</p>
                </div>
            </div>

            <!-- Right Column -->
            <div class="right-column">
                <!-- File Peningkatan Kualitas -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-file-download"></i> File Peningkatan Kualitas</h2>
                    </div>
                    <div class="file-card-content">
                        <i class="fas fa-file-pdf file-icon"></i>
                        <div class="file-name">{{ $peningkatan->file_pdf ?? 'Tidak ada file' }}</div>
                        <div class="btn-group">
                            @php
                                $filePath = $peningkatan->file_pdf
                                    ? asset('storage/' . trim($peningkatan->file_pdf))
                                    : null;
                            @endphp
                    ...
                    <!-- Status Peningkatan Kualitas -->
                    <div class="card" style="margin-top: 2rem;">
                        <div class="card-header">
                            <h2><i class="fas fa-check-circle"></i> Status Peningkatan Kualitas</h2>
                        </div>
                        <div class="related-regulations">
                            @if($peningkatan->mencabut)
                                <div class="status-item">
                                    <i class="fas fa-arrow-circle-right"></i>
                                    <div>
                                        <div class="status-label">Mencabut:</div>
                                        <ul>
                                            @foreach(explode("\n", str_replace("\r", '', $peningkatan->mencabut)) as $item)
                                                <li>{{ $item }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            openPopupBtn.forEach(link => {
                link.addEventListener('click', function (e) {
                    e.preventDefault();
                    const piagamId = this.dataset.id;

                    // Panggil endpoint detail Piagam SPI (pastikan rutenya sesuai di web.php)
                    fetch(`/piagamspi/detail/${piagamId}`)
                        .then(res => res.json())
                        .then(data => {
                            // Judul & Tahun
                        ...
                        .catch(err => console.error('Error fetching Piagam:', err));
                });
            });
