...
<main class="container">
    <div class="main-content">
        <div class="left-column">
            <!-- Ringkasan Pedoman MR -->
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-info-circle"></i> Ringkasan Pedoman MR</h2>
                    <a href="#" class="btn-primary openPopup" data-id="{{$pedomanmr->id }}">
                        Lihat Selengkapnya
                    </a>
                </div>
                <p class="abstract-content">
                    {{$pedomanmr->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                </p>
            </div>
            ...
            <div class="card" style="margin-top: 2rem;">
                <div class="card-header">
                    <h2><i class="fas fa-tags"></i> Metadata Pedoman MR</h2>
                </div>
                <dl class="metadata-grid">
                    <div class="metadata-item">
                        <dt>Tipe Dokumen</dt>
                        <dd>{{$pedomanmr->tipe_dokumen ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Judul</dt>
                        <dd>{{$pedomanmr->judul_meta ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>T.E.U.</dt>
                        <dd>{{$pedomanmr->teu ?? '-' }}</dd>
                    ...
                    <div class="metadata-item">
                        <dt>Nomor</dt>
                        <dd>{{$pedomanmr->nomor ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bentuk</dt>
                        <dd>{{$pedomanmr->bentuk ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bentuk Singkat</dt>
                        <dd>{{$pedomanmr->bentuk_singkat ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tahun</dt>
                        <dd>{{$pedomanmr->tahun_meta ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tempat Penetapan</dt>
                        <dd>{{$pedomanmr->tempat_penetapan ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Penetapan</dt>
                        <dd>{{$pedomanmr->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Pengundangan</dt>
                        <dd>{{$pedomanmr->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Berlaku</dt>
                        <dd>{{$pedomanmr->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Sumber</dt>
                        <dd>{{$pedomanmr->sumber ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Subjek</dt>
                        <dd>{{$pedomanmr->subjek ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Status</dt>
                        <dd>{{$pedomanmr->status ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bahasa</dt>
                        <dd>{{$pedomanmr->bahasa ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Lokasi</dt>
                        <dd>{{$pedomanmr->lokasi ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bidang</dt>
                        <dd>{{$pedomanmr->bidang ?? '-' }}</dd>
                    </div>
                </dl>
                <div class="social-share">
                    <span class="share-text">
                        Halaman ini telah diakses {{$pedomanmr->view_count ?? 0 }} kali
                    </span>
            ...
            <div class="card" style="margin-top: 2rem;">
                ...
                <p class="abstract-content">{{$pedomanmr->uji_materi ?? 'Belum Tersedia' }}</p>
            </div>
        </div>
        ...
        <div class="right-column">
            ...
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-file-download"></i> File Pedoman MR</h2>
                </div>
                <div class="file-card-content">
                    <i class="fas fa-file-pdf file-icon"></i>
                    <div class="file-name">{{$pedomanmr->file_pdf ?? 'Tidak ada file' }}</div>
                    <div class="btn-group">
                        @php
                            $filePath =$pedomanmr->file_pdf
                                ? asset('storage/' . trim($pedomanmr->file_pdf))
                                : null;
                        @endphp
                ...
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-check-circle"></i> Status Pedoman MR</h2>
                    </div>
                    <div class="related-regulations">
                        @if($pedomanmr->mencabut)
                            <div class="status-item">
                                <i class="fas fa-arrow-circle-right"></i>
                                <div>
                                    <div class="status-label">Mencabut:</div>
                                    <ul>
                                        @foreach(explode("\n", str_replace("\r", '',$pedomanmr->mencabut)) as $item)
                                            <li>{{ $item }}</li>
                                        @endforeach
                                    </ul>
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            openPopupBtn.forEach(link => {
                link.addEventListener('click', function (e) {
                    ...
                    fetch(`/pedomanmr/detail/${piagamId}`)
