...
<body>
   ...
   <div class="container">
    ...
    <div class="document-grid">
        @forelse($pedomanmrs as $pedomanmr)
            <div class="document-card">
                <div class="card-icon">
                    <i class="fa fa-file-alt"></i>
                </div>
                <div class="card-body">
                    <h3 class="card-title">{{ $pedomanmr->judul }}</h3>
                    <p class="card-subtitle">Tahun: {{ $pedomanmr->tahun ?? '-' }}</p>
                </div>
                <div class="card-actions">
                    <a href="{{ route('pedomanmr.show', $pedomanmr->id) }}" class="action-btn view-btn">
                        <i class="fa fa-eye"></i> Lihat
                    </a>

                    @if(Auth::check() && Auth::user()->role === 'admin')
                        <a href="{{ route('pedomanmr.edit', $pedomanmr->id) }}" class="action-btn edit-btn">
                            <i class="fa fa-edit"></i> Edit
                        </a>

                        <form action="{{ route('pedomanmr.destroy', $pedomanmr->id) }}" method="POST"
