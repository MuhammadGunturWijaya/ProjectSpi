    ...
    <main class="container">
        <div class="main-content">
            <div class="left-column">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-info-circle"></i> Ringkasan Perubahan</h2>
                        <a href="#" class="btn-primary openPopup" data-id="{{ $perubahan->id }}">
                            Lihat Selengkapnya
                        </a>
                    </div>
                    <p class="abstract-content">
                        {{ $perubahan->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                    </p>
                </div>
                ...
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-tags"></i> Metadata Perubahan</h2>
                    </div>
                    <dl class="metadata-grid">
                        <div class="metadata-item">
                            <dt>Tipe Dokumen</dt>
                            <dd>{{ $perubahan->tipe_dokumen ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Judul</dt>
                            <dd>{{ $perubahan->judul_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>T.E.U.</dt>
                            <dd>{{ $perubahan->teu ?? '-' }}</dd>
                        ...
                        <div class="metadata-item">
                            <dt>Nomor</dt>
                            <dd>{{ $perubahan->nomor ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk</dt>
                            <dd>{{ $perubahan->bentuk ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk Singkat</dt>
                            <dd>{{ $perubahan->bentuk_singkat ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tahun</dt>
                            <dd>{{ $perubahan->tahun_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tempat Penetapan</dt>
                            <dd>{{ $perubahan->tempat_penetapan ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Penetapan</dt>
                            <dd>{{ $perubahan->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Pengundangan</dt>
                            <dd>{{ $perubahan->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Berlaku</dt>
                            <dd>{{ $perubahan->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Sumber</dt>
                            <dd>{{ $perubahan->sumber ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Subjek</dt>
                            <dd>{{ $perubahan->subjek ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Status</dt>
                            <dd>{{ $perubahan->status ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bahasa</dt>
                            <dd>{{ $perubahan->bahasa ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Lokasi</dt>
                            <dd>{{ $perubahan->lokasi ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bidang</dt>
                            <dd>{{ $perubahan->bidang ?? '-' }}</dd>
                        </div>
                    </dl>
                    <div class="social-share">
                        <span class="share-text">
                            Halaman ini telah diakses {{ $perubahan->view_count ?? 0 }} kali
                        </span>
                ...
                <div class="card" style="margin-top: 2rem;">
                    ...
                    <p class="abstract-content">{{ $perubahan->uji_materi ?? 'Belum Tersedia' }}</p>
                </div>
            </div>
            ...
            <div class="right-column">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-file-download"></i> File Perubahan</h2>
                    </div>
                    <div class="file-card-content">
                        <i class="fas fa-file-pdf file-icon"></i>
                        <div class="file-name">{{ $perubahan->file_pdf ?? 'Tidak ada file' }}</div>
                        <div class="btn-group">
                            @php
                                $filePath = $perubahan->file_pdf
                                    ? asset('storage/' . trim($perubahan->file_pdf))
                                    : null;
                            @endphp
                    ...
                    <div class="card" style="margin-top: 2rem;">
                        <div class="card-header">
                            <h2><i class="fas fa-check-circle"></i> Status Perubahan</h2>
                        </div>
                        <div class="related-regulations">
                            @if($perubahan->mencabut)
                                <div class="status-item">
                                    <i class="fas fa-arrow-circle-right"></i>
                                    <div>
                                        <div class="status-label">Mencabut:</div>
                                        <ul>
                                            @foreach(explode("\n", str_replace("\r", '', $perubahan->mencabut)) as $item)
                                                <li>{{ $item }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
