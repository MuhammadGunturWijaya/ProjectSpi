...
<main class="container">
    <div class="main-content">
        <div class="left-column">
            <!-- Ringkasan Penguatan Akuntabilitas -->
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-info-circle"></i> Ringkasan Penguatan Akuntabilitas</h2>
                    <a href="#" class="btn-primary openPopup" data-id="{{ $penguatan->id }}">
                        Lihat Selengkapnya
                    </a>
                </div>
                <p class="abstract-content">
                    {{ $penguatan->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                </p>
            </div>

            <!-- Metadata Penguatan Akuntabilitas -->
            <div class="card" style="margin-top: 2rem;">
                <div class="card-header">
                    <h2><i class="fas fa-tags"></i> Metadata Penguatan Akuntabilitas</h2>
                </div>
                <dl class="metadata-grid">
                    <div class="metadata-item">
                        <dt>Tipe Dokumen</dt>
                        <dd>{{ $penguatan->tipe_dokumen ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Judul</dt>
                        <dd>{{ $penguatan->judul_meta ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>T.E.U.</dt>
                        <dd>{{ $penguatan->teu ?? '-' }}</dd>
                    ...
                    <div class="metadata-item">
                        <dt>Nomor</dt>
                        <dd>{{ $penguatan->nomor ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bentuk</dt>
                        <dd>{{ $penguatan->bentuk ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bentuk Singkat</dt>
                        <dd>{{ $penguatan->bentuk_singkat ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tahun</dt>
                        <dd>{{ $penguatan->tahun_meta ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tempat Penetapan</dt>
                        <dd>{{ $penguatan->tempat_penetapan ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Penetapan</dt>
                        <dd>{{ $penguatan->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Pengundangan</dt>
                        <dd>{{ $penguatan->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Tanggal Berlaku</dt>
                        <dd>{{ $penguatan->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Sumber</dt>
                        <dd>{{ $penguatan->sumber ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Subjek</dt>
                        <dd>{{ $penguatan->subjek ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Status</dt>
                        <dd>{{ $penguatan->status ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bahasa</dt>
                        <dd>{{ $penguatan->bahasa ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Lokasi</dt>
                        <dd>{{ $penguatan->lokasi ?? '-' }}</dd>
                    </div>
                    <div class="metadata-item">
                        <dt>Bidang</dt>
                        <dd>{{ $penguatan->bidang ?? '-' }}</dd>
                    </div>
                </dl>
                <div class="social-share">
                    <span class="share-text">
                        Halaman ini telah diakses {{ $penguatan->view_count ?? 0 }} kali
                    </span>
            ...
            <div class="card" style="margin-top: 2rem;">
                ...
                <p class="abstract-content">{{ $penguatan->uji_materi ?? 'Belum Tersedia' }}</p>
            </div>
        </div>

        <!-- Right Column -->
        <div class="right-column">
            <!-- File Penguatan Akuntabilitas -->
            <div class="card">
                <div class="card-header">
                    <h2><i class="fas fa-file-download"></i> File Penguatan Akuntabilitas</h2>
                </div>
                <div class="file-card-content">
                    <i class="fas fa-file-pdf file-icon"></i>
                    <div class="file-name">{{ $penguatan->file_pdf ?? 'Tidak ada file' }}</div>
                    <div class="btn-group">
                        @php
                            $filePath = $penguatan->file_pdf
                                ? asset('storage/' . trim($penguatan->file_pdf))
                                : null;
                        @endphp
                ...
                <!-- Status Penguatan Akuntabilitas -->
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-check-circle"></i> Status Penguatan Akuntabilitas</h2>
                    </div>
                    <div class="related-regulations">
                        @if($penguatan->mencabut)
                            <div class="status-item">
                                <i class="fas fa-arrow-circle-right"></i>
                                <div>
                                    <div class="status-label">Mencabut:</div>
                                    <ul>
                                        @foreach(explode("\n", str_replace("\r", '', $penguatan->mencabut)) as $item)
                                            <li>{{ $item }}</li>
                                        @endforeach
                                    </ul>
                                </div>
