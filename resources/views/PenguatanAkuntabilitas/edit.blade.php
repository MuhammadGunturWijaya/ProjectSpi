...
@section('content')
    ...
    <div class="text-center mb-5">
        <h2 class="fw-bold text-primary">
            <i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $penguatan->judul }}
        </h2>
        <p class="text-muted fs-6">
            Perbarui informasi dan metadata dokumen penguatan akuntabilitas dengan mudah.
        </p>
    </div>
    ...
    <form action="{{ route('penguatanAkuntabilitas.update', $penguatan->id) }}" method="POST"
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="judul" name="judul"
                                value="{{ old('judul', $penguatan->judul) }}" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="tahun" name="tahun"
                                value="{{ old('tahun', $penguatan->tahun) }}">
                        </div>
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            @foreach($metaFields as $field)
                                <div class="col-md-6">
                                    ...
                                    <input type="{{ $field['type'] ?? 'text' }}" class="form-control"
                                        ...
                                        value="{{ old($field['name'], $penguatan->{$field['name']} && !is_string($penguatan->{$field['name']}) ? $penguatan->{$field['name']}->format('Y-m-d') : $penguatan->{$field['name']}) }}">
                                </div>
                            @endforeach
                            ...
                            <div class="col-12">
                                ...
                                @if($penguatan->file_pdf)
                                    <p class="mt-2 text-muted">
                                        <i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                        <a href="{{ asset('storage/' . $penguatan->file_pdf) }}" target="_blank">
                                            {{ $penguatan->file_pdf }}
                                        </a>
                                    </p>
                                @endif
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="mencabut" name="mencabut"
                                    rows="3">{{ old('mencabut', $penguatan->mencabut) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-5">
                            ...
                            <a href="{{ route('penguatanAkuntabilitas.index') }}" class="btn btn-outline-secondary fw-semibold">
                                Batal
                            </a>
                        </div>
                    </div>
