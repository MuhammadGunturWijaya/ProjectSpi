    ...
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-primary">
                <i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $penataan->judul }}
            </h2>
            <p class="text-muted fs-6">Perbarui informasi dan metadata dokumen penataan tata kelola dengan mudah.</p>
        </div>
        ...
        <form action="{{ route('penataan.update', $penataan->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="judul" name="judul"
                                    value="{{ old('judul', $penataan->judul) }}" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="tahun" name="tahun"
                                    value="{{ old('tahun', $penataan->tahun) }}">
                            </div>
                ...
                <div class="col-lg-6">
                    <div class="card shadow-sm border-0 h-100">
                        <div class="card-header bg-gradient-info text-white">
                            <h5 class="card-title mb-0">
                                <i class="bi bi-info-circle-fill me-2"></i>Metadata & Lampiran
                            </h5>
                        </div>
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                @foreach($metaFields as $field)
                                    <div class="col-md-6">
                                        ...
                                        <input type="{{ $field['type'] ?? 'text' }}" class="form-control"
                                            ...
                                            value="{{ old($field['name'], $penataan->{$field['name']} && !is_string($penataan->{$field['name']}) ? $penataan->{$field['name']}->format('Y-m-d') : $penataan->{$field['name']}) }}">
                                    </div>
                                @endforeach
                                ...
                                <div class="col-12">
                                    ...
                                    @if($penataan->file_pdf)
                                        <p class="mt-2 text-muted">
                                            <i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                            <a href="{{ asset('storage/' . $penataan->file_pdf) }}" target="_blank">
                                                {{ $penataan->file_pdf }}
                                            </a>
                                        </p>
                                    @endif
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control" id="mencabut" name="mencabut" rows="3">
                                        {{ old('mencabut', $penataan->mencabut) }}
                                    </textarea>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end mt-5">
                                ...
                                <a href="{{ route('penataan.index') }}" class="btn btn-outline-secondary fw-semibold">
                                    Batal
                                </a>
                            </div>
                        </div>
