    ...
    <main class="container">
        <div class="main-content">
            <div class="left-column">
                <!-- Ringkasan Penataan Tata Kelola -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-info-circle"></i> Ringkasan Penataan Tata Kelola</h2>
                        <a href="#" class="btn-primary openPopup" data-id="{{ $penataan->id }}">
                            Lihat Selengkapnya
                        </a>
                    </div>
                    <p class="abstract-content">
                        {{ $penataan->abstrak ?? 'Tidak ada ringkasan tersedia.' }}
                    </p>
                </div>

                <!-- Metadata Penataan Tata Kelola -->
                <div class="card" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2><i class="fas fa-tags"></i> Metadata Penataan Tata Kelola</h2>
                    </div>
                    <dl class="metadata-grid">
                        <div class="metadata-item">
                            <dt>Tipe Dokumen</dt>
                            <dd>{{ $penataan->tipe_dokumen ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Judul</dt>
                            <dd>{{ $penataan->judul_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>T.E.U.</dt>
                            <dd>{{ $penataan->teu ?? '-' }}</dd>
                        ...
                        <div class="metadata-item">
                            <dt>Nomor</dt>
                            <dd>{{ $penataan->nomor ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk</dt>
                            <dd>{{ $penataan->bentuk ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bentuk Singkat</dt>
                            <dd>{{ $penataan->bentuk_singkat ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tahun</dt>
                            <dd>{{ $penataan->tahun_meta ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tempat Penetapan</dt>
                            <dd>{{ $penataan->tempat_penetapan ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Penetapan</dt>
                            <dd>{{ $penataan->tanggal_penetapan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Pengundangan</dt>
                            <dd>{{ $penataan->tanggal_pengundangan?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Tanggal Berlaku</dt>
                            <dd>{{ $penataan->tanggal_berlaku?->format('d F Y') ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Sumber</dt>
                            <dd>{{ $penataan->sumber ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Subjek</dt>
                            <dd>{{ $penataan->subjek ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Status</dt>
                            <dd>{{ $penataan->status ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bahasa</dt>
                            <dd>{{ $penataan->bahasa ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Lokasi</dt>
                            <dd>{{ $penataan->lokasi ?? '-' }}</dd>
                        </div>
                        <div class="metadata-item">
                            <dt>Bidang</dt>
                            <dd>{{ $penataan->bidang ?? '-' }}</dd>
                        </div>
                    </dl>
                    <div class="social-share">
                        <span class="share-text">
                            Halaman ini telah diakses {{ $penataan->view_count ?? 0 }} kali
                        </span>
                ...
                <div class="card" style="margin-top: 2rem;">
                    ...
                    <p class="abstract-content">{{ $penataan->uji_materi ?? 'Belum Tersedia' }}</p>
                </div>
            </div>

            <!-- Right Column -->
            <div class="right-column">
                <!-- File Penataan Tata Kelola -->
                <div class="card">
                    <div class="card-header">
                        <h2><i class="fas fa-file-download"></i> File Penataan Tata Kelola</h2>
                    </div>
                    <div class="file-card-content">
                        <i class="fas fa-file-pdf file-icon"></i>
                        <div class="file-name">{{ $penataan->file_pdf ?? 'Tidak ada file' }}</div>
                        <div class="btn-group">
                            @php
                                $filePath = $penataan->file_pdf
                                    ? asset('storage/' . trim($penataan->file_pdf))
                                    : null;
                            @endphp
                    ...
                    <!-- Status Penataan Tata Kelola -->
                    <div class="card" style="margin-top: 2rem;">
                        <div class="card-header">
                            <h2><i class="fas fa-check-circle"></i> Status Penataan Tata Kelola</h2>
                        </div>
                        <div class="related-regulations">
                            @if($penataan->mencabut)
                                <div class="status-item">
                                    <i class="fas fa-arrow-circle-right"></i>
                                    <div>
                                        <div class="status-label">Mencabut:</div>
                                        <ul>
                                            @foreach(explode("\n", str_replace("\r", '', $penataan->mencabut)) as $item)
                                                <li>{{ $item }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
