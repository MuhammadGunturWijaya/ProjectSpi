    ...
    <div class="container py-5">
        <div class="text-center mb-5">
            <h2 class="fw-bold text-primary">
                <i class="bi bi-pencil-square me-2"></i>Edit Dokumen {{ $Konsideran->judul }}
            </h2>
            <p class="text-muted fs-6">Perbarui informasi dan metadata dokumen Konsideran SPI dengan mudah.</p>
        </div>
        ...
        <form action="{{ route('konsideran.update', $Konsideran->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="judul" name="judul"
                                    value="{{ old('judul', $Konsideran->judul) }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="tahun" class="form-label fw-semibold">Tahun <span
                                        class="text-danger">*</span></label>
                                <input type="number" class="form-control" id="tahun" name="tahun"
                                    value="{{ old('tahun', $Konsideran->tahun) }}" required>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="kata_kunci" name="kata_kunci"
                                    value="{{ old('kata_kunci', $Konsideran->kata_kunci) }}">
                            </div>
                            <div class="mb-3">
                                <label for="abstrak" class="form-label fw-semibold">Abstrak</label>
                                <textarea class="form-control" id="abstrak" name="abstrak"
                                    rows="4">{{ old('abstrak', $Konsideran->abstrak) }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label for="catatan" class="form-label fw-semibold">Catatan</label>
                                <textarea class="form-control" id="catatan" name="catatan"
                                    rows="3">{{ old('catatan', $Konsideran->catatan) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                @foreach($metaFields as $field)
                                    <div class="col-md-6">
                                        ...
                                        <input type="{{ $field['type'] ?? 'text' }}" class="form-control"
                                            ...
                                            value="{{ old($field['name'], $Konsideran->{$field['name']} && !is_string($Konsideran->{$field['name']}) ? $Konsideran->{$field['name']}->format('Y-m-d') : $Konsideran->{$field['name']}) }}">
                                ...
                                <div class="col-12">
                                    ...
                                    @if($Konsideran->file_pdf)
                                        <p class="mt-2 text-muted"><i class="bi bi-file-earmark-pdf me-1"></i>File saat ini:
                                            <a href="{{ asset('storage/' . $Konsideran->file_pdf) }}"
                                                target="_blank">{{ $Konsideran->file_pdf }}</a>
                                        </p>
                                    @endif
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control" id="mencabut" name="mencabut"
                                        rows="3">{{ old('mencabut', $Konsideran->mencabut) }}</textarea>
                                </div>
                            </div>
